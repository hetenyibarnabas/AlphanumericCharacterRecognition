...
/// <summary>
/// Provides EMNIST images and labels as training samples.
/// </summary>
public class EmnistDataset: ICharacterDataset
{
    ...
    public EmnistDataset(string imagePath, string labelPath)
    {
        ...
        if (images.Length != labels.Length) throw new InvalidDataException("The number of images and labels does not match.");
    }
    ...
    public (Tensor Image, long Label) GetItem(int index)
    {
        ...
        for (int y = 0; y < sourceSize; y++)
        {
            ...
            {
                // This swaps row/column to correct EMNIST's stored orientation (it was inverted).
        ...
        Tensor imageTensor = tensor(rgb).reshape(3, targetSize, targetSize);
